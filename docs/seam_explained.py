"""Generate an ELI4 illustrated PDF explaining how SEAM works end-to-end."""
...
def page1_big_picture(pdf):
    fig = new_page(pdf, "Page 1: What Is SEAM Trying to Do?")
    ...
    txt(fig, 0.88,
        ...
        "SEAM's job: teach it to cooperate — without rebuilding the whole brain.",
    ...
    fig.text(0.5, 0.07,
             "SEAM = add a tiny helper on top of the frozen brain that says:\n"
...
def page3_unet(pdf):
    ...
    txt(fig, 0.88,
        ...
        "SEAM only adds a thin sticky note on top.",
        fc="#cce5ff", ec="#007bff", size=11)
...
def page4_heuristic_phi(pdf):
    ...
    txt(fig, 0.89,
        "SEAM adds a HELPER that computes a 'danger score' (called phi, φ) for every robot.\n"
...
def page10_cheatsheet(pdf):
    ...
    txt(fig, 0.11,
        ...
        "To generate PDFs:  python docs/seam_explained.py",
        fc="#d4edda", ec="#28a745", size=11)
...
def main():
    import os
    out_path = os.path.join(os.path.dirname(__file__), "seam_explained.pdf")
